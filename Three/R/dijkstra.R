...
#'
#' @param wiki_graph A data.frame representing the adjacency matrix or edge list of the graph. The graph should contain edge weights.
...
#'
#' @references \href{https://en.wikipedia.org/wiki/Dijkstra\'s_algorithm}{Dijkstra's Algorithm - Wikipedia}
#' 
#' @examples
...
dijkstra <- function(wiki_graph, init_node){
  if(!is.data.frame(wiki_graph)){
    stop()
  }
  if(length(wiki_graph) != 3){
    stop()
  }
  if(!identical(names(wiki_graph), c("v1", "v2", "w"))){
    stop()
  }
  ##create graph
  node <- unique(wiki_graph[[1]])
  node_num <- length(node)
  
  if(init_node > node_num){
    stop()
  }
  ...
  for(edge_index in seq_along(wiki_graph[[3]])) {
    graph_matrix[wiki_graph[[1]][edge_index],wiki_graph[[2]][edge_index]] <- wiki_graph[[3]][edge_index]
  }
